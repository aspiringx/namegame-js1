    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            CREATE TABLE \`group_users\`(
                ...
                \`group_id\` int NULL,
                \`user_id\` int NULL,
                \`role\` varchar(255) NULL COMMENT 'Role of user for permission pruposes.',
                \`title\` varchar(255) NULL COMMENT 'Title of user in group. Leader, president, etc.',
                \`member_since\` timestamp NULL COMMENT 'When user joined up. May be before created_at.',
                \`is_leader\` tinyint NULL COMMENT 'Leaders are visible to group members before personal connections.' DEFAULT 0,
                \`is_active\` tinyint NOT NULL COMMENT 'Must be active to be visible.' DEFAULT 1,
                \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
                \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
                INDEX \`IDX_36620c8747186b00c458893c59\` (\`user_id\`, \`group_id\`),
                UNIQUE INDEX \`REL_be6db0d7dabab05d97233d19f6\` (\`group_id\`),
                UNIQUE INDEX \`REL_eba8af4e65056abb4c5f62556c\` (\`user_id\`),
